/**
 * Service API pour la gestion des livreurs
 *
 * Gère les opérations liées aux livreurs :
 * - Récupération de la liste des livreurs avec pagination et recherche
 */

import apiClient from "./axiosConfig";

/**
 * Interface pour un livreur - Structure réelle de l'API
 */
...
/**
 * Interface pour la réponse paginée des livreurs - Structure réelle de l'API
 */
 ...
 */
export interface NormalizedLivreursResponse {
  data: Livreur[];
  meta: {
    current_page: number;
    from: number;
...
/**
 * Récupère la liste des livreurs avec pagination et recherche
 * @param page - Numéro de page (par défaut: 1)
 * @param search - Terme de recherche optionnel
 * @returns Promise avec les données paginées
 */
async function getLivreurs(page: number = 1, search?: string): Promise<NormalizedLivreursResponse> {
  try {
    ...
    const response = await apiClient.get<LivreursResponse | string>(`/livreurs?${params.toString()}`);
    ...
    const apiResponse = response.data as LivreursResponse;
